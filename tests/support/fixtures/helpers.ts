...
// ЕДИНЫЙ генератор даты/времени для тестов. Все даты отсчитываются от «сегодня»
// в днях: futureDate(2) — через 2 дня, futureDate(365) — через год,
// futureDate(-1) — вчера (для «прошлых»). Час по умолчанию 10:00; точное время
// задаётся через hour/minute/second. Формат: YYYY-MM-DDTHH:MM[:SS] (секунды
// добавляются только когда заданы явно, чтобы не ломать существующие сравнения).
export function futureDate(daysFromNow: number, opts: FutureDateOptions = {}): string {
  ...
  const time = second > 0
    ? `${H}:${Mi}:${String(second).padStart(2, '0')}`
    : `${H}:${Mi}`;
  return `${Y}-${M}-${D}T${time}`;
}
...
// Прогон плановых уведомлений (правила 2/3/4). DEBUG-эндпоинт, csrf_exempt.
// user_id — ограничить обработку одним пользователем (изоляция тестов).
// now — опционально, YYYY-MM-DDTHH:MM[:SS] (для детерминированных напоминаний).
export async function runNotifications(page: Page, now?: string, userId?: number): Promise<RunNotificationsResult> {
  const body: Record<string, unknown> = {};
  if (now) body.now = now;
  if (userId) body.user_id = userId;
  const r = await page.request.post('/api/auth/test/run-notifications/', { data: body });
  if (!r.ok) throw new Error(`runNotifications: HTTP ${r.status} — ${await r.text()}`);
  return r.json();
}

// Установка оставшихся занятий активного абонемента пользователя (DEBUG, staff).
// Нужен staff-логин. userId ищет активный абонемент клиента.
export async function setSubscriptionRemaining(
  page: Page,
  userId: number,
  remaining: number,
): Promise<void> {
  const csrf = await getCsrf(page);
  const result = await page.evaluate(async ({ userId, remaining, csrf }) => {
    const r = await fetch('/api/auth/test/set-subscription-remaining/', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-CSRFToken': csrf },
      credentials: 'same-origin',
      body: JSON.stringify({ user_id: userId, remaining }),
    });
    if (!r.ok) throw new Error(`setSubscriptionRemaining: HTTP ${r.status} — ${await r.text()}`);
    return r.json();
  }, { userId, remaining, csrf });
}

// Список уведомлений текущего пользователя.
export async function getNotifications(page: Page): Promise<Array<{ id: number; type: string; title: string; message: string; is_read: boolean }>> {
  const r = await page.request.get('/api/notifications/');
  if (!r.ok) throw new Error(`getNotifications: HTTP ${r.status}`);
  const data = await r.json();
