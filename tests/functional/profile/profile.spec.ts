...
test.describe('Страница профиля', { tag: ['@functional'] }, () => {
  ...
  test.describe('Навигация — нижняя панель', { tag: ['@functional'] }, () => {
  ...
  test.describe('Модальное окно редактирования', { tag: ['@functional'] }, () => {
  ...
  test.describe('Редактирование профиля', { tag: ['@functional'] }, () => {
  ...
  test.describe('Редактирование — безопасность', { tag: ['@functional', '@security'] }, () => {
  ...
  test.describe('Пустые состояния', { tag: ['@functional'] }, () => {
  ...
  test.describe('Наполненные состояния (студент 5)', { tag: ['@functional'] }, () => {
    ...
    test('история посещений показывает прошлые занятия и тепловую карту', async ({ page }) => {
      ...
      const eventDate = futureDate(364).split('T')[0];
      let eventId = 0;
      try {
  ...
  test.describe('Модальное окно истории посещений', { tag: ['@functional'] }, () => {
  ...
  test.describe('Модальное окно подтверждения', { tag: ['@functional'] }, () => {
    test('модалка подтверждения скрыта по умолчанию', async ({ page }) => {
      await expect(page.getByTestId('confirm-modal')).toBeHidden();
    });
  });

  test.describe('Активный абонемент на профиле', { tag: ['@functional'] }, () => {
  ...
  test.describe('История баланса', { tag: ['@functional', '@regression'] }, () => {
    ...
    test('использование абонемента за занятие появляется в истории', async ({ page }) => {
      ...
      await registerNewUser(page, phone);
      const uid = (await getUserInfo(page, phone)).id!;
      const eventDate = futureDate(3).split('T')[0];
      let eventId = 0;
      try {
        await page.context().clearCookies();
        await loginAsAdmin(page);
    ...
    test('списание с баланса и возврат за занятие появляются в истории', async ({ page }) => {
      ...
      await registerNewUser(page, phone);
      const uid = (await getUserInfo(page, phone)).id!;
      const eventDate = futureDate(5).split('T')[0];
      let eventId = 0;
      try {
        await page.context().clearCookies();
        await loginAsAdmin(page);
  ...
  test.describe('Предстоящие занятия на профиле', { tag: ['@functional'] }, () => {
  ...
  test.describe('Секция достижений', { tag: ['@functional'] }, () => {
  ...
  test.describe('Секция соцсетей', { tag: ['@functional'] }, () => {
  ...
  test.describe('VK секция', { tag: ['@functional', '@api'] }, () => {
    ...
    test('для привязанного VK с выключенными уведомлениями кнопка «Включить уведомления ВК» открывает модалку', async ({ page }) => {
      const phone = uniquePhone();
      await registerNewUser(page, phone);
      try {
        await attachVk(page, `test-vk-${Date.now()}`);
        await page.goto('/profile/');
        await expect(page.getByText('VK ID привязан')).toBeVisible();
        await expect(page.getByTestId('vk-notify-status')).toContainText('не активны');

        const btn = page.getByTestId('vk-notify-toggle-btn');
        await expect(btn).toContainText('Включить уведомления ВК');
        await btn.click();

        await expect(page.getByTestId('vk-notify-modal')).toBeVisible();
        await expect(page.getByTestId('vk-notify-message')).toHaveValue(/хочу получать уведомления/);
        await expect(page.getByTestId('vk-notify-open')).toBeVisible();
      } finally {
        await deleteTestUser(page, phone);
      }
    });

    test('кнопка «Открыть чат с ботом» включает уведомления ВК', async ({ page }) => {
      const phone = uniquePhone();
      await registerNewUser(page, phone);
      try {
        await attachVk(page, `test-vk-${Date.now()}`);
        ...
        await page.getByTestId('vk-notify-toggle-btn').click();
        await expect(page.getByTestId('vk-notify-modal')).toBeVisible();

        // Открытие в новой вкладке дало бы popup; для теста вызываем enable напрямую.
        await page.evaluate(() => (window as any).__vkEnableNotifications && (window as any).__vkEnableNotifications());

        await expect(page.getByTestId('vk-notify-status')).toContainText('включены', { timeout: 5000 });
        await expect(page.getByTestId('vk-notify-toggle-btn')).toContainText('Отключить уведомления ВК');
      } finally {
        await deleteTestUser(page, phone);
      }
    });

    test('для привязанного VK с включёнными уведомлениями кнопка «Отключить» выключает их', async ({ page }) => {
      const phone = uniquePhone();
      await registerNewUser(page, phone);
      try {
        await attachVk(page, `test-vk-${Date.now()}`);
        // Включить уведомления заранее
        await page.evaluate(async () => {
          const r = await fetch('/api/auth/vk/toggle/', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'X-CSRFToken': (window as any).getCsrfToken() },
            credentials: 'same-origin',
            body: JSON.stringify({ action: 'toggle_notifications' }),
          });
          return r.json();
        });
        await page.goto('/profile/');
        await expect(page.getByTestId('vk-notify-status')).toContainText('включены');

        await page.getByTestId('vk-notify-toggle-btn').click();
        await expect(page.getByTestId('vk-notify-status')).toContainText('не активны', { timeout: 5000 });
        await expect(page.getByTestId('vk-notify-toggle-btn')).toContainText('Включить уведомления ВК');
      } finally {
        await deleteTestUser(page, phone);
      }
    });
  ...
  test.describe('Аватар', { tag: ['@functional'] }, () => {
  ...
  test.describe('Выход', { tag: ['@functional'] }, () => {
  ...
  test.describe('Скрытые формы', { tag: ['@functional'] }, () => {
