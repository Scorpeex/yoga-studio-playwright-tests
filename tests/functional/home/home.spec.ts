...
import { installWidgetStub, getWidgetLog } from '../../support/fixtures/widget-stub';
import { getResolvedTestTariffs } from '../../support/fixtures/helpers';
...
test.describe('Главная страница (дашборд)', { tag: ['@functional'] }, () => {
  ...
  test.describe('Модалка пополнения баланса', { tag: ['@functional', '@regression'] }, () => {
  ...
  test.describe('Модалка истории посещений', { tag: ['@functional'] }, () => {
  ...
  test.describe('Модалка покупки абонемента', { tag: ['@functional', '@regression'] }, () => {
  ...
  test.describe('Модалка продления абонемента', { tag: ['@functional', '@regression'] }, () => {
    ...
    test('продление: кнопка, модалка, нехватка средств и успешное продление', async ({ page }) => {
      ...
      // Стартовый баланс покрывает покупку абонемента и продление (не ниже цены после первой покупки).
      const seedBalance = 2 * LITE_SUBSCRIPTION_PRICE + 1000;
      const phone = uniquePhone();
      await registerNewUser(page, phone);
      const uid = (await getUserInfo(page, phone)).id!;
      ...
      try {
        ...
        expect(parseFloat(state.balance)).toBe(seedBalance - LITE_SUBSCRIPTION_PRICE);
        ...
        await expect(page.locator('#renewBalance')).toHaveText(formatRu(seedBalance - LITE_SUBSCRIPTION_PRICE));
        ...
        expect(parseFloat(after.balance)).toBe(seedBalance - LITE_SUBSCRIPTION_PRICE);
        await expect(page.getByTestId('renew-btn')).toHaveCount(0);
      } finally {
        await page.context().clearCookies();
        await loginAsAdmin(page);
  ...
  test.describe('Stories (новости)', { tag: ['@functional'] }, () => {
  ...
  test.describe('Уведомления', { tag: ['@functional'] }, () => {
    ...
    test('клик вне dropdown закрывает его', async ({ page }) => {
      await page.getByTestId('notification-bell').click();
      await expect(page.locator('.notification-dropdown.show')).toBeVisible();
      await page.locator('.greeting-text').click();
  ...
  test.describe('Модалка ближайших занятий', { tag: ['@functional'] }, () => {
    ...
    test('кнопка открывает модалку, запись снимает занятие из списка', async ({ page }) => {
      ...
      await registerNewUser(page, phone);
      const uid = (await getUserInfo(page, phone)).id!;
      const eventDate = futureDate(1).split('T')[0];
      let eventId = 0;
      try {
        await page.context().clearCookies();
        await loginAsAdmin(page);
  ...
  test.describe('YooKassa виджет', { tag: ['@functional', '@api'] }, () => {
