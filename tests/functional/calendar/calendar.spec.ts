...
import {
  ...
  assertEventMovedTo,
  dragTimegridLater,
  resizeTimegridLonger,
  getEventDurationMinutes,
  getBackendDurationMinutes,
  eventResizeHandleCount,
} from './drag-helpers';
...
test.describe('Календарь — отображение и навигация', { tag: ['@functional'] }, () => {
  test.beforeEach(async ({ page }) => {
    await loginAsStudent(page);
    await page.goto('/calendar/');
  });

  test.describe('Отображение', { tag: ['@functional'] }, () => {
  ...
  test.describe('Нижняя панель навигации', { tag: ['@functional'] }, () => {
  ...
  test.describe('Навигация по датам', { tag: ['@functional'] }, () => {
  ...
  test.describe('Переключение вида', { tag: ['@functional'] }, () => {
  ...
  test.describe('Модальные окна — скрыты по умолчанию', { tag: ['@functional'] }, () => {
  ...
  test.describe('Вкладки модального окна', { tag: ['@functional'] }, () => {
...
test.describe('Легенда залов — фильтрация расписания', { tag: ['@functional'] }, () => {
  // Залы из seed-данных: 1 — «Зал №1 (Основной)», 2 — «Зал №2 (Малый)», 3 — «VIP Зал».
  // Создаём по одному событию в двух разных залах на одной чистой дате один раз (beforeAll),
  // чтобы проверять отображение/скрытие через контент события (в нём выводится имя зала: «| <hall_name>»).
  const HALL_1_NAME = 'Зал №1 (Основной)';
  const HALL_2_NAME = 'Зал №2 (Малый)';
  let eventIds: number[] = [];

  function eventsOnCleanDate(page: import('@playwright/test').Page) {
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    try {
      ...
      // Два события на один день: зал 1 в 10:00, зал 2 в 11:00
      const a = await createCalendarEvent(page, {
        class_type_id: 1,
        start: futureDate(CLEAN_EVENT_DAY + 0),
        duration: 60,
        hall_id: 1,
        tariff_id: 3,
        max_participants_override: 10,
      });
      const b = await createCalendarEvent(page, {
        class_type_id: 1,
        start: futureDate(CLEAN_EVENT_DAY).slice(0, 10) + 'T11:00',
        duration: 60,
        hall_id: 2,
        tariff_id: 3,
        max_participants_override: 10,
  ...
  test('по умолчанию видны все залы', async ({ page }) => {
    const events = eventsOnCleanDate(page);
    await expect(events).toHaveCount(2);
    await expect(page.locator('.hall-legend-item.active')).toHaveCount(0);
  });

  test('клик по залу оставляет только занятия этого зала', async ({ page }) => {
    await page.getByTestId('hall-legend-item').filter({ hasText: HALL_1_NAME }).click();
    await expect(page.locator('.hall-legend-item.active')).toHaveCount(1);
    const events = eventsOnCleanDate(page);
    await expect(events).toHaveCount(1); // осталось только занятие выбранного зала
    await expect(events.filter({ hasText: HALL_1_NAME })).toBeVisible();
    await expect(events.filter({ hasText: HALL_2_NAME })).toHaveCount(0); // занятие другого зала скрыто
  });

  test('переключение на другой зал меняет фильтр', async ({ page }) => {
    await page.getByTestId('hall-legend-item').filter({ hasText: HALL_1_NAME }).click();
    await page.getByTestId('hall-legend-item').filter({ hasText: HALL_2_NAME }).click();
    await expect(page.locator('.hall-legend-item.active')).toHaveCount(1);
    const events = eventsOnCleanDate(page);
    await expect(events).toHaveCount(1);
    await expect(events.filter({ hasText: HALL_2_NAME })).toBeVisible();
    await expect(events.filter({ hasText: HALL_1_NAME })).toHaveCount(0);
  });

  test('повторный клик по активному залу сбрасывает фильтр', async ({ page }) => {
    await page.getByTestId('hall-legend-item').filter({ hasText: HALL_1_NAME }).click();
    await expect(page.locator('.hall-legend-item.active')).toHaveCount(1);
    await expect(eventsOnCleanDate(page)).toHaveCount(1);
    await page.getByTestId('hall-legend-item').filter({ hasText: HALL_1_NAME }).click();
    await expect(page.locator('.hall-legend-item.active')).toHaveCount(0);
    await expect(eventsOnCleanDate(page)).toHaveCount(2);
    await expect(eventsOnCleanDate(page).filter({ hasText: HALL_2_NAME })).toBeVisible();
  });

  test('выбор фильтра сбрасывается после обновления страницы', async ({ page }) => {
    await page.getByTestId('hall-legend-item').filter({ hasText: HALL_1_NAME }).click();
    await expect(page.locator('.hall-legend-item.active')).toHaveCount(1);
    await expect(eventsOnCleanDate(page)).toHaveCount(1);
    await page.reload();
...
test.describe('Студент — взаимодействие с календарём', { tag: ['@functional'] }, () => {
...
test.describe('Студент — несколько событий', { tag: ['@functional'] }, () => {
...
test.describe('Студент — запись/отмена по представлениям', { tag: ['@functional'] }, () => {
  let eventId: number;
...
test.describe('Студент — ограничения и граничные случаи', { tag: ['@functional', '@regression'] }, () => {
  ...
  test.describe('Списание и возврат средств', { tag: ['@functional', '@api', '@regression'] }, () => {
    const STUDENT_ID = 5;
...
test.describe('Удаление занятия возвращает средства', { tag: ['@functional', '@regression'] }, () => {
  const STUDENT_ID = 5;
  ...
  test('удаление серии возвращает средства со всех занятий', async ({ page }) => {
    ...
    const start = futureDate(CAL_DAY_OFFSET, { hour: 9 });
...
test.describe('Запись других пользователей', { tag: ['@functional', '@security'] }, () => {
...
test.describe('Сплит-тариф', { tag: ['@functional', '@api', '@regression'] }, () => {
...
test.describe('Админ — краевые случаи посещаемости', { tag: ['@functional', '@api'] }, () => {
  const STUDENT_ID = 5;
  ...
  test('после удаления администратором можно записать клиента снова (повторное списание)', async ({ page }) => {
    ...
    const start = futureDate(CAL_DAY_OFFSET, { hour: 10 });
    const startDate = start.slice(0, 10);
    await cleanupEventsOnDate(page, startDate);
    const result = await createCalendarEvent(page, {
      class_type_id: 1,
  ...
  test('обновление посещаемости не меняет отменённые записи', async ({ page }) => {
    ...
    const start = futureDate(CAL_DAY_OFFSET, { hour: 11 });
    const startDate = start.slice(0, 10);
    await cleanupEventsOnDate(page, startDate);
    const result = await createCalendarEvent(page, {
      class_type_id: 1,
...
test.describe('Администратор — управление календарём', { tag: ['@functional'] }, () => {
  ...
  test('предупреждение при сохранении без зала', async ({ page }) => {
    await openCreateModal(page, 21);

    await page.getByTestId('event-class-type').selectOption('1');
    await page.getByTestId('event-tariff').selectOption(String(TARIFF_GROUP_ID));
    await page.getByTestId('event-hall').selectOption('');
    await expect(page.locator('#hallWarning')).toBeHidden();
    ...
    // Модалка остаётся открытой, предупреждение видно, у зала красная рамка
    await expect(page.locator('#eventModal')).toBeVisible();
    await expect(page.locator('#hallWarning')).toBeVisible();
    await expect(page.locator('#hallWarning')).toContainText('Зал не выбран');
    await expect(page.getByTestId('event-hall')).toHaveCSS('border-color', 'rgb(220, 53, 69)');

    // Запрос на создание не отправлялся
    expect(createRequests).toHaveLength(0);

    // После выбора зала предупреждение и красная рамка исчезают
    await page.getByTestId('event-hall').selectOption('2');
    await expect(page.locator('#hallWarning')).toBeHidden();
    await expect(page.getByTestId('event-hall')).not.toHaveCSS('border-color', 'rgb(220, 53, 69)');
  });

  test('предупреждение при сохранении без типа занятия', async ({ page }) => {
    await openCreateModal(page, 22);

    await page.getByTestId('event-hall').selectOption('2');
    await page.getByTestId('event-tariff').selectOption(String(TARIFF_GROUP_ID));
    await page.getByTestId('event-class-type').selectOption('');
    await expect(page.locator('#classTypeWarning')).toBeHidden();
    ...
    // Модалка остаётся открытой, предупреждение видно, у типа занятия красная рамка
    await expect(page.locator('#eventModal')).toBeVisible();
    await expect(page.locator('#classTypeWarning')).toBeVisible();
    await expect(page.locator('#classTypeWarning')).toContainText('Тип занятия не выбран');
    await expect(page.getByTestId('event-class-type')).toHaveCSS('border-color', 'rgb(220, 53, 69)');

    // Запрос на создание не отправлялся
    expect(createRequests).toHaveLength(0);

    // После выбора типа занятия предупреждение и красная рамка исчезают
    await page.getByTestId('event-class-type').selectOption('1');
    await expect(page.locator('#classTypeWarning')).toBeHidden();
    await expect(page.getByTestId('event-class-type')).not.toHaveCSS('border-color', 'rgb(220, 53, 69)');
  });

  test('серверное требование полей при создании занятия', async ({ page }) => {
    const csrf = await page.evaluate(() => {
      const m = document.cookie.match(/csrftoken=([^;]+)/);
      return m ? m[1] : '';
    });
    // Попытка создать занятие без зала и тарифа через API
    const res = await page.evaluate(async (token: string) => {
      const r = await fetch('/api/calendar/events/create/', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-CSRFToken': token },
        body: JSON.stringify({
          class_type_id: 1,
          start: '2030-09-01T10:00:00',
        }),
      });
    ...
    // Без зала
    const res2 = await page.evaluate(async (token: string) => {
      const r = await fetch('/api/calendar/events/create/', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-CSRFToken': token },
        body: JSON.stringify({
    ...
    expect(res2.body.error).toBe('Зал обязателен');

    // Без тарифа
    const res3 = await page.evaluate(async (token: string) => {
      const r = await fetch('/api/calendar/events/create/', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-CSRFToken': token },
        body: JSON.stringify({
    ...
    expect(res3.body.error).toBe('Тариф обязателен');

    // Без типа занятия
    const res4 = await page.evaluate(async (token: string) => {
      const r = await fetch('/api/calendar/events/create/', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-CSRFToken': token },
        body: JSON.stringify({
    ...
    expect(res4.body.error).toBe('Тип занятия обязателен');

    // Без даты
    const res5 = await page.evaluate(async (token: string) => {
      const r = await fetch('/api/calendar/events/create/', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-CSRFToken': token },
        body: JSON.stringify({
    ...
    expect(res5.body.error).toBe('Дата начала обязательна');
  });
  ...
  test('редактирование занятия через форму сохраняется на бэке', async ({ page }) => {
    const start = futureDate(CAL_DAY_OFFSET, { hour: 14 });
    const startDate = start.slice(0, 10);
    await cleanupEventsOnDate(page, startDate);
    const result = await createCalendarEvent(page, {
      class_type_id: 1,
  ...
  test('удаление занятия через интерфейс', async ({ page }) => {
    ...
    // Одиночное занятие: клик «Удалить» открывает модалку подтверждения,
    // а не удаляет молча — модалка редактирования остаётся открытой.
  ...
  test('удаление серии занятий через интерфейс', async ({ page }) => {
    const start = futureDate(CAL_DAY_OFFSET, { hour: 16 });
    const startDate = start.slice(0, 10);
    await cleanupEventsOnDate(page, startDate);
    const result = await createCalendarEvent(page, {
      class_type_id: 1,
  ...
  test('одиночное занятие становится серией через редактирование (API)', async ({ page }) => {
    const hour = 10;
    const weekDates = [0, 7, 14, 21].map((d) => futureDate(CAL_DAY_OFFSET + d, { hour }).slice(0, 10));
  ...
  test('повторное сохранение серии не создаёт дубликаты', async ({ page }) => {
    const hour = 15;
    const weekDates = [0, 7, 14].map((d) => futureDate(CAL_DAY_OFFSET + d, { hour }).slice(0, 10));
    for (const d of weekDates) {
      await cleanupEventsOnDate(page, d);
    }
    const created = await createCalendarEvent(page, {
  ...
  test('отмена в модалке подтверждения удаления', async ({ page }) => {
    const start = futureDate(CAL_DAY_OFFSET, { hour: 12 });
    const startDate = start.slice(0, 10);
    await cleanupEventsOnDate(page, startDate);
    const result = await createCalendarEvent(page, {
      class_type_id: 1,
  ...
  test('подтверждение при удалении одиночного занятия: отмена', async ({ page }) => {
    const start = futureDate(CAL_DAY_OFFSET, { hour: 15 });
    const startDate = start.slice(0, 10);
    await cleanupEventsOnDate(page, startDate);
    const result = await createCalendarEvent(page, {
      class_type_id: 1,
    ...
    // Клик по «Удалить» открывает модалку подтверждения, а не удаляет молча
    await page.getByTestId('event-delete-btn').click();
    await expect(page.locator('#confirmModal')).toBeVisible();
    await expect(page.locator('#confirmMessage')).toContainText('Удалить');

    // Отмена — DELETE не уходит, занятие остаётся
    const deleteRequests: string[] = [];
    page.on('request', (r) => {
      if (r.url().includes(`/api/calendar/events/${eventId}/delete/`) && r.method() === 'POST') {
    ...
    await waitForEvents(page);
    expect(await getEventProp(page, eventId, 'duration')).toBe(60);
  });

  test('подтверждённое удаление одиночного занятия', async ({ page }) => {
    const start = futureDate(CAL_DAY_OFFSET, { hour: 16 });
    const startDate = start.slice(0, 10);
    await cleanupEventsOnDate(page, startDate);
    const result = await createCalendarEvent(page, {
    ...
    const deleteResponse = page.waitForResponse(
      (r) => r.url().includes(`/api/calendar/events/${eventId}/delete/`) && r.request().method() === 'POST',
      { timeout: 10000 },
    );
    await page.getByTestId('confirm-ok-btn').click();
    await expect(page.locator('#confirmModal')).toBeHidden();
    ...
    await waitForEvents(page);
    expect(await getEventProp(page, eventId, 'duration')).toBeNull();
  });

  test('предупреждение о числе записей при удалении занятия', async ({ page }) => {
    const start = futureDate(CAL_DAY_OFFSET, { hour: 17 });
    const startDate = start.slice(0, 10);
    await cleanupEventsOnDate(page, startDate);
    const result = await createCalendarEvent(page, {
      class_type_id: 1,
    ...
    // Готовим студента для записи и записываем его на занятие
    await deleteUserSubscriptions(page, STUDENT_ID);
    await setAllowedTariffs(page, STUDENT_ID, [TARIFF_GROUP_ID, 3]);
    const balanceBefore = parseFloat((await getUserInfo(page, '+73333333333')).balance!);
    const csrf = await getCsrfToken(page);
    const addRes = await page.request.post(`/api/calendar/events/${eventId}/attendance/add/`, {
      data: { client_id: STUDENT_ID },
      headers: { 'X-CSRFToken': csrf },
    ...
    expect((await addRes.json()).success).toBe(true);
    // После записи средства списаны
    const balanceAfterEnroll = parseFloat((await getUserInfo(page, '+73333333333')).balance!);
    expect(balanceAfterEnroll).toBeLessThan(balanceBefore);

    await navigateToDate(page, startDate);
    await page.locator(`.fc-daygrid-day[data-date="${startDate}"] .fc-event`).first().click();
    await expect(page.locator('#eventModal')).toBeVisible();

    // Предупреждение показывает число записей и возврат средств
    await page.getByTestId('event-delete-btn').click();
    await expect(page.locator('#confirmModal')).toBeVisible();
    await expect(page.locator('#confirmMessage')).toContainText('1 запись');
    await expect(page.locator('#confirmMessage')).toContainText('возвращены');

    // Подтверждаем удаление — запись отменяется, занятие исчезает
    await page.getByTestId('confirm-ok-btn').click();
    await expect(page.locator('#confirmModal')).toBeHidden();
    await waitForEvents(page);
    await expect.poll(() => getEventProp(page, eventId, 'duration'), { timeout: 10000 }).toBeNull();

    // Средства возвращены на баланс студента
    const balanceAfterDelete = parseFloat((await getUserInfo(page, '+73333333333')).balance!);
  ...
  test('удаление серии с записями: предупреждение и возврат средств', async ({ page }) => {
    const start = futureDate(CAL_DAY_OFFSET, { hour: 18 });
    const startDate = start.slice(0, 10);
    await cleanupEventsOnDate(page, startDate);
    const result = await createCalendarEvent(page, {
      class_type_id: 1,
    ...
    // Готовим студента и записываем его на первое занятие серии
    await deleteUserSubscriptions(page, STUDENT_ID);
    await setAllowedTariffs(page, STUDENT_ID, [TARIFF_GROUP_ID, 3]);
    const balanceBefore = parseFloat((await getUserInfo(page, '+73333333333')).balance!);
    const csrf = await getCsrfToken(page);
    const addRes = await page.request.post(`/api/calendar/events/${mainId}/attendance/add/`, {
      data: { client_id: STUDENT_ID },
      headers: { 'X-CSRFToken': csrf },
    });
    expect((await addRes.json()).success).toBe(true);
    ...
    // Модалка серии показывает предупреждение о записях и возврате средств
    await page.getByTestId('event-delete-btn').click();
    await expect(page.locator('#deleteConfirmModal')).toBeVisible();
    await expect(page.locator('#deleteSeriesBookingInfo')).toBeVisible();
    await expect(page.locator('#deleteSeriesBookingInfo')).toContainText('1 запись');
    await expect(page.locator('#deleteSeriesBookingInfo')).toContainText('возвращены');
    ...
    await waitForEvents(page);
    await expect.poll(() => getEventProp(page, mainId, 'duration'), { timeout: 10000 }).toBeNull();
    const remaining = await page.evaluate((targetIds) => {
      const evs = (window as any).calendar?.getEvents() || [];
      return evs.filter((e: any) => targetIds.includes(Number(e.id))).length;
    }, otherIds);
  ...
  test('поиск клиентов без результатов', async ({ page }) => {
    const start = futureDate(CAL_DAY_OFFSET, { hour: 13 });
    const startDate = start.slice(0, 10);
    await cleanupEventsOnDate(page, startDate);
    const result = await createCalendarEvent(page, {
      class_type_id: 1,
...
test.describe('Перетаскивание занятия (drag & drop)', { tag: ['@functional'] }, () => {

  test.describe('Студент — запрет', { tag: ['@functional', '@security'] }, () => {
    let eventId: number;
  ...
  test.describe('Администратор — разрешено', { tag: ['@functional'] }, () => {
    let eventId: number;
...
test.describe('Изменение длительности занятия (resize)', { tag: ['@functional'] }, () => {
  test.describe('Студент — запрет', { tag: ['@functional', '@security'] }, () => {
    let eventId: number;
    ...
    test('изменение длительности в недельном виде недоступно', async ({ page }) => {
      await navigateToView(page, 'week');
      await page.waitForTimeout(300);
      const event = page.locator(`.fc-timegrid-col[data-date="${CLEAN_EVENT_DATE}"] .fc-event`).first();
      await expect(event).toBeVisible();
      // У студента eventDurationEditable=false → resize-хендл не рендерится
      expect(await eventResizeHandleCount(event)).toBe(0);
      await assertForbidden(page, eventId);
    });

    test('изменение длительности в дневном виде недоступно', async ({ page }) => {
      await navigateToView(page, 'day');
      await page.waitForTimeout(300);
  ...
  test.describe('Администратор — разрешено', { tag: ['@functional'] }, () => {
    ...
    test('изменение длительности в недельном виде сохраняется на бэке', async ({ page }) => {
      const before = await getBackendDurationMinutes(page, eventId);
      expect(before).toBe(60);
      const event = page.locator(`.fc-timegrid-col[data-date="${CLEAN_EVENT_DATE}"] .fc-event`).first();
      await expect(event).toBeVisible();
      expect(await eventResizeHandleCount(event)).toBeGreaterThan(0);

      await resizeTimegridLonger(page, event, eventId, 30);

      await expect.poll(() => getEventDurationMinutes(page, eventId), { timeout: 10000 }).toBe(90);
      await expect.poll(() => getBackendDurationMinutes(page, eventId), { timeout: 10000 }).toBe(90);
    });
  });
});
...
test.describe('Посещаемость — роли и приватность', { tag: ['@functional', '@security'] }, () => {
...
test.describe('Сплит — оплата по дедлайну', { tag: ['@functional', '@api', '@regression'] }, () => {
...
test.describe('Абонемент — просроченный', { tag: ['@functional', '@regression'] }, () => {
...
test.describe('Списание из абонемента — повторная запись', { tag: ['@functional', '@regression'] }, () => {
...
test.describe('Группы тарифов (Lite/Pro)', { tag: ['@functional', '@api'] }, () => {
...
test.describe('Права студента', { tag: ['@functional', '@security'] }, () => {
...
test.describe('API событий — доступность тарифов', { tag: ['@functional', '@api'] }, () => {
...
test.describe('Цвет текста события зала', { tag: ['@functional', '@api'] }, () => {
  const HALL_ID = 2;
  let testEventId: number;
