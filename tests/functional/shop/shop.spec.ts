...
test.describe('Магазин (Shop)', { tag: ['@functional'] }, () => {
  ...
  test.describe('Модалка пополнения', { tag: ['@functional'] }, () => {
  ...
  test.describe('Модалка покупки абонемента', { tag: ['@functional'] }, () => {
    test('модалка скрыта по умолчанию', async ({ page }) => {
  ...
  test.describe('Отображение недоступных тарифов', { tag: ['@functional'] }, () => {
  ...
  test.describe('Покупка абонемента', { tag: ['@functional', '@regression'] }, () => {
    ...
    test('повторная покупка при активном абонементе возвращает ошибку', async ({ page }) => {
      const phone = await freshUserWithBalance(page, 2 * GROUP_SUB_PRICE, [GROUP_ID, 3]);
  ...
  test.describe('Пополнение баланса (валидация)', { tag: ['@functional', '@api', '@regression'] }, () => {
  ...
  test.describe('Вебхук YooKassa', { tag: ['@functional', '@api', '@regression'] }, () => {
    ...
    test('renew_subscription добавляет занятия и продлевает срок', async ({ page }) => {
      ...
      // → остаётся 1 → продление доступно. Стартовый баланс покрывает покупку + продление.
      const seedBalance = 2 * LITE_SUBSCRIPTION_PRICE + 1000;
      const phone = await freshUserWithBalance(page, seedBalance, [LITE_ID]);
      const info = await getUserInfo(page, phone);
      const id = info.id!;
      ...
      try {
        ...
        expect(after.balance).toBe((seedBalance - 2 * LITE_SUBSCRIPTION_PRICE).toFixed(2));
  ...
  test.describe('Пополнение из модалки покупки', { tag: ['@functional'] }, () => {
